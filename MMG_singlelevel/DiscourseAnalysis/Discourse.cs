...
namespace MMG
{
	...
	public class Discourse
	{
        ...
        public bool IsProperNoun(ParseNode parseNode)
        {
            if (parseNode.Children != null && parseNode.Children.Count == 1 && ((ParseNode)parseNode.Children[0]).Goal=="PPN")
            {
                return true;
            }
            return false;
		...
		public void Begin(ArrayList ParseTrees)
		{
			...
			foreach(ParseTree ps in ParseTrees)
			{
				...
				while(nodes.Count>0)
				{
					...
					{
                        string s = SentenceParser.GetWordString(ps,pn);
						...
						else
						{
							foreach(ParseNode ch in pn.Children)
								...
								{
                                    string strsdbs = SentenceParser.GetWordString(ps, ch);
				...
				for(int i=0;i<PronounNPs.Count;i++)
				{
					...
					{
                        string sdgsjhgds = SentenceParser.GetWordString((ParseTree)ParseTrees[((DiscourseEntry)de).TreeNum], ((DiscourseEntry)de).Node);
                    ...
                    string s = SentenceParser.GetWordString(ps,maxEntry.Node);
					if(maxEntry!=null)
					{
						...
						if(items1==null||items2==null||items1!=items2)
						{
                            ...
                            s = SentenceParser.GetWordString((ParseTree)ParseTrees[((DiscourseEntry)maxEntry).TreeNum], ((DiscourseEntry)maxEntry).Node);
                            /////////////////////
                            DiscourseEntry de0 = (DiscourseEntry)newitems[0];
                            for (int j = 1; j < newitems.Count; j++)
                            {
                             DiscourseEntry de = (DiscourseEntry)newitems[j];
                             de.Ag.Type = de0.Ag.Type;
                            }
                            /////////////////////
            ...
            )
		{
			...
			else
			{
				...
				if(Lexicon.IsPlural(word))
					...
					if(WSDReplace.IsHuman(sense))//here we will send the sense //New
						this.Type=1;
					else
						this.Type=2;
