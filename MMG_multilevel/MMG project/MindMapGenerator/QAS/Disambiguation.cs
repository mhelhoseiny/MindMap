    ...
    class Disambiguation
    {
        ...
        private void PrepareInputArrOfDisambiguation()
        {
            ...
            foreach (string s in WordInfoArr2)
            {
                ...
                if (strarr[0] == "VING")
                {
                    if (strarr[1].ToLower() == "reproducing")
                    {
                        mwi[counter] = new MyWordInfo(strarr[1], Wnlib.PartsOfSpeech.Noun);
                        counter++;
                    }
                    else if (strarr[1].ToLower() == "treating" || counter > 0 && mwi[counter - 1].Pos == Wnlib.PartsOfSpeech.Verb)
                    {
                ...
                else if (strarr[0] == "V" || strarr[0] == "VPSP" || strarr[0] == "BE1" || strarr[0] == "VINF")
                {
                    mwi[counter] = new MyWordInfo(strarr[1], Wnlib.PartsOfSpeech.Verb);
                    counter++;
                }
        ...
        public ArrayList ArrRepeatedPS = new ArrayList();
        public ArrayList ArrIndxs = new ArrayList();
        private void checkParseTreeRepitition(bool deleteRepeatedParseTrees)
        {
            ArrRepeatedPS.Clear();
            ArrIndxs.Clear();
            bool flag = false;
            ...
            if (flag2)
            {
                ...
                for (int k = 1; k < SParseTrees.Count; k++)
                {
                    ...
                    if (deleteRepeatedParseTrees && current == prev)
                    {
