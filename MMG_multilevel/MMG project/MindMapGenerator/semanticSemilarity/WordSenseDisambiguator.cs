...
namespace WordsMatching
{
	...
	public class MyWordInfo
	{
        ...
        public MyWordInfo(string word, Wnlib.PartsOfSpeech pos, int sense)
        {
            this.Word = word;
            this.Pos = pos;
            this.Sense = sense;
        ...
        {
            return Word + "(" + Pos.ToString() + Sense.ToString() +")";
        }
	}
	...
	public class WordSenseDisambiguator
	{
        ...
        /// <returns></returns>
        public MyWordInfo[] MMG_Disambiguate(MyWordInfo[] words,out double score)
        {
            ArrayList ArrScore = new ArrayList();
            _contextWords = words;
            MyInit();
            ArrScore = MMG_Scoring_Overlaps();
            for (int i = 0; i < _contextWords.Length; i++)
                _contextWords[i].Sense = _bestSenses[i];
            score = 0;
            foreach (int i in ArrScore)
            {
                    score += i;
            }
            return _contextWords;
