...
namespace OntologyConceptsEditor
{
    public partial class frmOntologyEditor : Form
    {
        ...
        private void FillCombo()
        {
            ...
            string[] itArr = items.ToArray();
            cmbConceptNames.Items.AddRange(itArr);
            cmbFirstConcept.Items.AddRange(itArr);
            cmbSecondConcept.Items.AddRange(itArr);
            sr.Close();
        }
        ...
        private void cmbConceptNames_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            lblCurrentConcept.Text = "Current Concept  --->  " + cmbConceptNames.Text;
            GetData(zzz);
        }
...
//            List<string> leftPath, RightPath;
        ...
        private void btnAddFiller_Click(object sender, EventArgs e)
        {
            //OntologyConceptsEditor.projectData.path
            string name = cmbConceptNames.Text;
            string firstletter = name.Substring(0, 1);
            string _path = OntologyConceptsEditor.projectData.path + @"\" + firstletter + @"\" + name + ".txt";

            FileStream fs = new FileStream(_path, FileMode.Append, FileAccess.Write);
            StreamWriter sr = new StreamWriter(fs);

            string newLine = txtFillerName.Text.ToUpper() + "\t\tVALUE\t" + txtFillerValue.Text.ToUpper() + "\n\n";
        ...
        {
            if (char.IsLetter(Convert.ToChar(txtNewConceptName.Text.Substring(0, 1))))
            {
                string name = txtNewConceptName.Text;
                string firstletter = name.Substring(0, 1);
                string _path = OntologyConceptsEditor.projectData.path + @"\" + firstletter + @"\" + name + ".txt";
                ...
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    ...
                    {
                        string newLine = row.Cells[0].Value.ToString().ToUpper() + "\t\tVALUE\t" + row.Cells[1].Value.ToString().ToUpper() + "\n\n";
                        sr.WriteLine(newLine);
                    }
                }
                sr.Close();
